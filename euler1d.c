...
struct UpdateStruct update_struct_new(int num_zones, int block_size, real x0, real x1)
{
    ...
    update.primitive = (real*) compute_malloc(num_zones * 4 * sizeof(real));
    update.conserved = (real*) compute_malloc(num_zones * 4 * sizeof(real));
    update.flux = (real*) compute_malloc((num_zones + 1) * 4 * sizeof(real));
